...
def get_neighbor_finder(data, uniform, max_node_idx=None):
    ...
    for source, destination, edge_idx, timestamp, label in zip(data.sources, data.destinations,
        ...
        if type(destination) is list or type(destination) is np.ndarray:
            for dest, lab in zip(destination, label):
                if lab > 0:
                    adj_list[source].append((dest, edge_idx, timestamp))
                    adj_list[dest].append((source, edge_idx, timestamp))
        else:
...
class NeighborFinder:
    def __init__(self, adj_list, uniform=False, seed=None, is_online=True):
        ...
        if not is_online:
            for neighbors in adj_list:
                ...
                self.node_to_neighbors.append(np.array([0] + [x[0] for x in sorted_neighhbors]))
                self.node_to_edge_idxs.append(np.array([0] + [x[1] for x in sorted_neighhbors]))
                self.node_to_edge_timestamps.append(np.array([0] + [x[2] for x in sorted_neighhbors]))
    ...
    def reset_finder(self):
        if self.is_online:
            ...
            self.node_to_neighbors = [np.array([0]) for i in range(self.n_nodes)]
            self.node_to_edge_idxs = [np.array([0]) for i in range(self.n_nodes)]
            self.node_to_edge_timestamps = [np.array([0]) for i in range(self.n_nodes)]

            self.neg_node_to_neighbors = []
            self.neg_node_to_edge_idxs = []
            self.neg_node_to_edge_timestamps = []
            self.neg_node_to_neighbors = [np.array([0]) for i in range(self.n_nodes)]
            self.neg_node_to_edge_idxs = [np.array([0]) for i in range(self.n_nodes)]
            self.neg_node_to_edge_timestamps = [np.array([0]) for i in range(self.n_nodes)]

    def set_node_neighbors(self, src_idx, neighbors, edge_idxs, timestamps):
        ### TODO check that timestamps is sorted
        self.node_to_neighbors[src_idx] = np.hstack([self.node_to_neighbors[src_idx], neighbors])
        self.node_to_edge_idxs[src_idx] = np.hstack([self.node_to_edge_idxs[src_idx], edge_idxs])
        self.node_to_edge_timestamps[src_idx] = np.hstack([self.node_to_edge_timestamps[src_idx], timestamps])

    def set_neg_node_neighbors(self, src_idx, neighbors, edge_idxs, timestamps):
        ### TODO check that timestamps is sorted
        self.neg_node_to_neighbors[src_idx] = np.hstack([self.neg_node_to_neighbors[src_idx], neighbors])
        self.neg_node_to_edge_idxs[src_idx] = np.hstack([self.neg_node_to_edge_idxs[src_idx], edge_idxs])
        self.neg_node_to_edge_timestamps[src_idx] = np.hstack([self.neg_node_to_edge_timestamps[src_idx], timestamps])

    def update_graphs(self, src, dst, edges, ts):
        if self.is_online:
            for node in set(src):
                self.set_node_neighbors(node, dst[src == node], edges[src == node], ts[src == node])

    def update_negative_graphs(self, src, dst, edges, ts):
        if self.is_online:
            for node in set(src):
                self.set_neg_node_neighbors(node, dst[src == node], edges[src == node], ts[src == node])
    ...
    def find_before(self, src_idx, cut_time):
        ...
        return self.node_to_neighbors[src_idx][1:i], self.node_to_edge_idxs[src_idx][1:i], self.node_to_edge_timestamps[
                                                                                             src_idx][1:i]

    def find_neg_before(self, src_idx, cut_time):
        """
        Extracts all the interactions happening before cut_time for user src_idx in the overall interaction graph. The returned interactions are sorted by time.
        ...
        """
        i = np.searchsorted(self.neg_node_to_edge_timestamps[src_idx], cut_time)

        return self.neg_node_to_neighbors[src_idx][1:i], self.neg_node_to_edge_idxs[src_idx][1:i], self.neg_node_to_edge_timestamps[
                                                                                             src_idx][1:i]
    ...
    def get_neg_temporal_neighbor(self, source_nodes, timestamps, n_neighbors=20):
        """
        Given a list of users ids and relative cut times, extracts a sampled temporal neighborhood of each user in the list.
        ...
        for i, (source_node, timestamp) in enumerate(zip(source_nodes, timestamps)):
            source_neighbors, source_edge_idxs, source_edge_times = self.find_neg_before(source_node,
                                                                                     timestamp)  # extracts all neighbors, interactions indexes and timestamps of all interactions of user source_node happening before cut_time
    ...
    def get_random_walk(self, src_idx, cut_time, seed=0, len_walk=5, e_idx=None, packed=False, recent_bias=1.0):
        ...
        if packed:
            ...
            for hop in range(len_walk):
                n_idx_l, e_idx_l, ts_l = self.find_before(cur_n_idx, cur_time)
                cur_len = len(n_idx_l)
                if cur_len == 0:
